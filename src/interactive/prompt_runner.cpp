...
static void run_condition_editor(ScreenInteractive& screen, std::shared_ptr<ConditionData>& cond) {
    ...
    for (;;) {
        ...
        if (!cond) {
            ...
            if (type_sel == 0) {
                ...
                /* NOT is unary: only "Edit sub", no "Add sub-condition" */
        ...
        if (exit_after_create) continue;  // Rebuild layout so "Add sub-condition" etc. appear
        if (exit_done) return;
        if (exit_reset) { cond = nullptr; return; }
        if (exit_edit_sub) { run_condition_editor(screen, *exit_edit_sub); continue; }
        if (exit_remove && cond) {
        ...
        if (exit_add_sub && cond && type_sel != 3) {
            /* NOT is unary: only AND/OR get "Add sub-condition"; never add a second sub to NOT */
...
void run_components_screen(ScreenInteractive& screen, MetadataBuilder& b) {
    ...
    // Component loop: type-aware form (only fields relevant to selected type)
    std::vector<std::string> comp_types = { "executable", "library", "external", "variant", "layer" };
    int comp_type_sel = 0;
    std::string comp_id, source, dest, deps_str, conan_ref;
    ...
    while (has_more) {
        ...
        // Executable section: source, dest, deps, optional filters
        ...
        auto exec_filter_block = Container::Vertical({
            exec_filter_inc_i,
            exec_filter_exc_i,
            exec_fm_radio
        });
        auto exec_section = Container::Vertical({
            ...
            exec_dest_i,
            exec_deps_i,
            exec_show_filters_cb,
            Maybe(exec_filter_block, [&] { return show_filters; })
        });

        // Library section: library type, hierarchical, source, dest, deps, optional filters, optional git
        ...
        auto lib_filter_block = Container::Vertical({
            lib_filter_inc_i,
            lib_filter_exc_i,
            lib_fm_radio
        });
        ...
        auto lib_git_block = Container::Vertical({
            lib_git_url_i,
            lib_git_tag_i,
            lib_git_branch_i,
            lib_git_commit_i
        ...
        auto lib_section = Container::Vertical({
            ...
            lib_use_git_cb,
            Maybe(lib_git_block, [&] { return use_git; })
        });

        // External section: Conan ref only
        auto conan_i = Input(&conan_ref, "e.g. freertos/11.0.1");
        auto ext_section = Container::Vertical({ conan_i });

        // Variant section: source, dest, variations (subdir + condition), then deps
        auto var_src_i = Input(&source, ".");
        auto var_dest_i = Input(&dest, "e.g. apps/variant");
        auto var_subdir_i = Input(&var_subdir, "subdir name");
        auto edit_cond_btn = Button("Edit condition", [&] { run_condition_editor(screen, current_condition); });
        ...
        // Layer section: dest, subdirs, deps
        ...
        auto layer_section = Container::Vertical({
            layer_dest_i,
            subdirs_i,
            layer_deps_i
        });
        ...
        auto add_comp_btn = Button("Add this component", [&] {
            ...
            if (c.type == "external") {
                c.conan_ref = conan_ref;
            }
            if (c.type == "variant") {
                c.variations = var_list;
            }
        ...
        auto comp_render = Renderer(comp_layout, [&]() -> Element {
            ...
            if (comp_type_sel == 0) {
                lines.push_back(text("Source: "));
                lines.push_back(exec_src_i->Render());
                lines.push_back(text("Dest: "));
                lines.push_back(exec_dest_i->Render());
                lines.push_back(text("Dependencies: "));
                lines.push_back(exec_deps_i->Render());
                lines.push_back(exec_show_filters_cb->Render());
                if (show_filters) {
                    lines.push_back(text("Filter include: "));
                    lines.push_back(exec_filter_inc_i->Render());
                    lines.push_back(text("Filter exclude: "));
                    lines.push_back(exec_filter_exc_i->Render());
                    lines.push_back(text("Filter mode: "));
                    lines.push_back(exec_fm_radio->Render());
                }
            } else if (comp_type_sel == 1) {
                lines.push_back(text("Library type: "));
                lines.push_back(lib_radio->Render());
                lines.push_back(lib_hierarchical_cb->Render());
                lines.push_back(text("Source: "));
                lines.push_back(lib_src_i->Render());
                lines.push_back(text("Dest: "));
                lines.push_back(lib_dest_i->Render());
                lines.push_back(text("Dependencies: "));
                lines.push_back(lib_deps_i->Render());
                lines.push_back(lib_show_filters_cb->Render());
                if (show_filters) {
                    lines.push_back(text("Filter include: "));
                    lines.push_back(lib_filter_inc_i->Render());
                    lines.push_back(text("Filter exclude: "));
                    lines.push_back(lib_filter_exc_i->Render());
                    lines.push_back(text("Filter mode: "));
                    lines.push_back(lib_fm_radio->Render());
                }
                lines.push_back(lib_use_git_cb->Render());
                if (use_git) {
                    lines.push_back(text("Git URL: "));
                    lines.push_back(lib_git_url_i->Render());
                    lines.push_back(text("Git tag: "));
                    lines.push_back(lib_git_tag_i->Render());
                    lines.push_back(text("Git branch: "));
                    lines.push_back(lib_git_branch_i->Render());
                    lines.push_back(text("Git commit: "));
                    lines.push_back(lib_git_commit_i->Render());
                }
            } else if (comp_type_sel == 2) {
                lines.push_back(text("Conan ref: "));
                lines.push_back(conan_i->Render());
            } else if (comp_type_sel == 3) {
                lines.push_back(text("Source: "));
                lines.push_back(var_src_i->Render());
                lines.push_back(text("Dest: "));
                lines.push_back(var_dest_i->Render());
                lines.push_back(text("Variation subdir: "));
                lines.push_back(var_subdir_i->Render());
                ...
                lines.push_back(text("Dependencies: "));
                lines.push_back(var_deps_i->Render());
            } else {
                lines.push_back(text("Dest: "));
                lines.push_back(layer_dest_i->Render());
                lines.push_back(text("Subdirs (component ids): "));
                lines.push_back(subdirs_i->Render());
                lines.push_back(text("Dependencies: "));
                lines.push_back(layer_deps_i->Render());
            }
        ...
        if (comp_type_sel == 3 && var_edit_index >= 0 && static_cast<size_t>(var_edit_index) < var_list.size()) {
            run_condition_editor(screen, var_list[static_cast<size_t>(var_edit_index)].condition);
            reset_form = false;
            continue;
        }
